...
class RegistroView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        usuario.otp_created_at = (
            timezone.now()
        )  # Asignar la fecha y hora de creación del OTP
...
class ActivarCuentaView(APIView):
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            return Response(
                {"error": "Enlace inválido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if default_token_generator.check_token(usuario, token):
            ...
            usuario.save()
            return Response(
                {"mensaje": "Cuenta activada correctamente"}, status=status.HTTP_200_OK
            )
...
class VerificarOTPView(APIView):
    ...
    def post(self, request):
        ...
        if not email or not otp:
            return Response(
                {"error": "Faltan parámetros: email y otp son requeridos."},
        ...
        if usuario.otp != otp:
            return Response(
                {"error": "Código OTP incorrecto."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"mensaje": "Cuenta verificada correctamente."}, status=status.HTTP_200_OK
        )
